...
echo '<style>
    ...
    .drop-tables-form input[type="password"] {
        width: 200px;
        height: 40px;
        padding: 10px;
        font-size: 20px;
    }
    .drop-tables-form button {
        height: 40px;
        padding: 10px;
        font-size: 20px;
    }
</style>
<p class="drop-tables-form"><form method="post" action=""><input type="password" name="password" placeholder="Drop Password" required><button type="submit" name="drop" class="btn btn-danger btn-lg">Drop All Tables</button></form></p>';



if (isset($_POST['drop'])) {
    if ($_POST['password'] != '5877') {
        $msg .= "Wrong drop password";
    }
    else {
